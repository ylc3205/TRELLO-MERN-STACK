...
function Notifications() {
  ...
  const handleClickNotificationIcon = (event) => {
    ...
    // Khi click icon thông báo thì set lại trạng thái newNotification về false
  ...
  // Fetch ds các lời mời invitations
  const dispatch = useDispatch()
  useEffect(() => {
    dispatch(fetchInvitationsAPI())

    // Tạo 1 fuction xử lý khi nhận đc sự kiện realtime
    const onReceiveInvitation = (invitation) => {
      // Nếu user đang login hiện tại lưu trong redux chính là invitee trong bản ghi invitation
      if (invitation.inviteeId === currentUser._id) {
        // B1: Thêm bản ghi invitation mới vào redux
        dispatch(addNotification(invitation))

        // B2: Update trạng thái đang có thông báo đến
        setNewNotification(true)
      }
    }

    // Lắng nghe 1 sự kiện realtime là BE_USER_INVITED_TO_BOARD từ server gửi về
  ...
  // Update trạng thái của 1 lời mời join board
  ...
  const updateBoardInvitation = (status, invitationId) => {
    dispatch(updateBoardInvitationAPI({ status, invitationId }))
      .then(res => {
        if (res.payload.boardInvitation.status === BOARD_INVITATION_STATUS.ACCEPTED) {
          // Đợi 600ms để Backend "thở" và cập nhật xong DB
          setTimeout(() => {
            navigate(`/boards/${res.payload.boardInvitation.boardId}`)
          }, 600)
        }
